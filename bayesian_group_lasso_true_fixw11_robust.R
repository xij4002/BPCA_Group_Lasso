...
sample_group_lasso_loadings <- function(X_residual, Z, sigma2, tau_g_squared, group_id, G, fixed_loadings = NULL) {
  P <- ncol(X_residual)
  K <- ncol(Z)
  ...
  # Apply fixed loadings constraints
  if (!is.null(fixed_loadings)) {
    for (constraint in fixed_loadings) {
      if (is.list(constraint) && all(c("row", "col", "val") %in% names(constraint))) {
        W[constraint$row, constraint$col] <- constraint$val
      }
    }
  }
