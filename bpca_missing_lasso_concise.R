...
mcmc_bpca_group_lasso <- function(X, K, G, n_iter, burn_in, n_chains = 4, 
                                  mu_prior_var = 1000, 
  ...
  for (chain in 1:n_chains) {
    ...
    # PCA/SVD-based smart initialization for faster mixing
    mu <- colMeans(X_work, na.rm = TRUE)
    X_centered <- sweep(X_work, 2, mu)
    svd_init <- try(svd(X_centered, nu = min(N, K), nv = min(P, K)), silent = TRUE)
    if (inherits(svd_init, "try-error")) {
      Z <- matrix(rnorm(N * K, 0, 0.1), N, K)
      W <- matrix(rnorm(P * K, 0, 0.1), P, K)
    } else {
      d_k <- svd_init$d[1:K]
      U_k <- svd_init$u[, 1:K, drop = FALSE]
      V_k <- svd_init$v[, 1:K, drop = FALSE]
      sqrt_d <- diag(sqrt(pmax(d_k, 1e-8)), nrow = K, ncol = K)
      Z <- U_k %*% sqrt_d
      W <- V_k %*% sqrt_d
    }
    ...
    for (iter in 1:n_iter) {
      ...
      Sigma_z <- solve(t(W) %*% W / sigma2 + diag(K))
      mu_z_prefix <- Sigma_z %*% t(W) / sigma2
      for (i in 1:N) {
        mu_z <- mu_z_prefix %*% (X_work[i, ] - mu)
        Z[i, ] <- as.vector(mvtnorm::rmvnorm(1, mu_z, Sigma_z))
      }
      
      X_residual_W <- X_work - matrix(rep(mu, each = N), nrow = N)
      W <- sample_group_lasso_loadings(X_residual_W, Z, sigma2, tau_g2, group_id, G)

      # Post-update sign anchoring to reduce label/sign switching across iterations
      for (k in 1:K) {
        if (sum(W[, k], na.rm = TRUE) < 0) {
          W[, k] <- -W[, k]
          Z[, k] <- -Z[, k]
        }
      }
...
align_factor_chains <- function(W_chains, Z_chains, method = "procrustes") {
  ...
  if (n_chains < 2) {
    cat("Only one chain found. No alignment needed.\n")
    return(list(W_aligned = W_chains, Z_aligned = Z_chains))
  }
  
  cat("Aligning", n_chains, "chains using", method, "method (Stephens 2000)...\n")

  if (method != "procrustes") {
    stop("Only 'procrustes' method is currently supported.")
  }
...
## ======== DEBUG SCALE CORRECTION HELPERS (ADDED) ========
...
## ======== END DEBUG SCALE CORRECTION HELPERS ========
