...
mcmc_bpca_group_lasso <- function(X, K, G, n_iter, burn_in, n_chains = 3, 
    ...
    for (iter in 1:n_iter) {
      ...
      if (use_block_sampling && iter %% 3 == 1 && K > 1) {
        ...
        W <- sample_group_lasso_loadings(
            ...
            fix_w11_value = fix_w11_value
        )
      }
      
      # ================================================================= #
      # ===== 以下的符号翻转代码块已被移除 (SIGN-FLIPPING BLOCK REMOVED) ===== #
      # ================================================================= #
  ...
  W_samples_aligned <- W_samples_all[, 1:post_burn_iters, , , drop = FALSE]
  Z_samples_aligned <- Z_samples_all[, 1:post_burn_iters, , , drop = FALSE]
  
  if (is.na(fix_w11_value) && n_chains > 1) {
  ...
  if (n_chains > 1) {
    ...
    W_samples_3d <- W_samples_aligned[1, , , ]
    Z_samples_3d <- Z_samples_aligned[1, , , ]
    W_mean <- apply(W_samples_3d, c(2, 3), mean)
    Z_mean <- apply(Z_samples_3d, c(2, 3), mean)
  }
  mu_mean <- apply(mu_samples_all[, 1:post_burn_iters, , drop = FALSE], 3, mean)
