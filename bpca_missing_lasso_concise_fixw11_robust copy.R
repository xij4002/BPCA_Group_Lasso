...
mcmc_bpca_group_lasso <- function(X, K, G, n_iter, burn_in, n_chains = 3, thinning = 10,
                                  mu_prior_var = 1000, 
  ...
  # Calculate the number of samples to store after thinning
  post_burn_in_iters <- n_iter - burn_in
  n_saved_samples <- floor(post_burn_in_iters / thinning)

  Z_samples_all <- array(NA, dim = c(n_chains, n_saved_samples, N, K))
  W_samples_all <- array(NA, dim = c(n_chains, n_saved_samples, P, K))
  mu_samples_all <- array(NA, dim = c(n_chains, n_saved_samples, P))
  sigma2_samples_all <- matrix(NA, n_chains, n_saved_samples)
  tau_g2_samples_all <- array(NA, dim = c(n_chains, n_saved_samples, G))
  lambda2_samples_all <- matrix(NA, n_chains, n_saved_samples)
  ...
  for (chain in 1:n_chains) {
    ...
    saved_idx <- 1 # Initialize counter for saved samples
    for (iter in 1:n_iter) {
      ...
      if (use_block_sampling && iter %% 3 == 1 && K > 1) {
        ...
        W <- sample_group_lasso_loadings(
            X_residual_W, Z, sigma2, tau_g2, group_id, G,
      ...
      if (is.na(fix_sigma2)) {
        ...
        shape_post_sigma <- sigma_a + (N * P) / 2
        scale_post_sigma <- sigma_b + sse / 2
      ...
      # Thinning logic: only save if iteration is past burn-in AND is a multiple of thinning interval
      if (iter > burn_in && (iter - burn_in) %% thinning == 0) {
        if (saved_idx <= n_saved_samples) {
            Z_samples_all[chain, saved_idx, , ] <- Z
            W_samples_all[chain, saved_idx, , ] <- W
            mu_samples_all[chain, saved_idx, ] <- mu
            sigma2_samples_all[chain, saved_idx] <- sigma2
            tau_g2_samples_all[chain, saved_idx, ] <- tau_g2
            lambda2_samples_all[chain, saved_idx] <- lambda2
  ...
  W_samples_aligned <- W_samples_all
  Z_samples_aligned <- Z_samples_all
  
  if (n_chains > 1) {
    aligned_results <- align_factor_chains(W_samples_aligned, Z_samples_aligned)
  ...
  if (n_chains > 1) {
    ...
    W_samples_3d <- W_samples_aligned[1, , , , drop = FALSE]
    Z_samples_3d <- Z_samples_aligned[1, , , , drop = FALSE]
    W_mean <- apply(W_samples_3d, c(3, 4), mean)
    Z_mean <- apply(Z_samples_3d, c(3, 4), mean)
  }
  mu_mean <- apply(mu_samples_all, 3, mean)
  sigma2_mean <- mean(sigma2_samples_all)
  ...
  return(list(
    ...
    mu_samples = mu_samples_all,
    sigma2_samples = sigma2_samples_all,
    tau_g2_samples = tau_g2_samples_all,
    lambda2_samples = lambda2_samples_all
  ))
}
...
calculate_convergence_diagnostics <- function(
  ...
  W_rhats <- sapply(w_indices, function(idx) {
    if (idx[1] <= P && idx[2] <= K) {
      # Check if this is W[1,1] and if it's constrained
      if (idx[1] == 1 && idx[2] == 1) {
        w11_samples <- W_chains[, , 1, 1]
        w11_var <- var(as.vector(w11_samples), na.rm = TRUE)
        if (w11_var < 1e-10) {
          cat("   Loading W[1,1]                 R-hat: N/A (constrained)\n")
          return(NA)
        }
      }
